...
- `npm run typecheck`、`npm test`、`npm run build`、`npm run test:i18n-catalogue`、`npm run test:i18n-ui`、`npm run test:workflow-state-ui`、`npm run test:lab-features-ui`、`npm run test:print-native`、`npm run test:print-ui`、`npm run test:window-tabs`、`npm run test:page-text-edit-ui`、`git diff --check` 全部通过。
