...
```powershell
.\scripts\package-windows.ps1 2.0.11
```

macOS：

```bash
bash scripts/package-macos.sh 2.0.11
```
...
2.0.11 自动批注 / Automatic Annotation：发布验证必须覆盖全文与当前选区两种范围；文档开头、邻近段落及跨页文字只能作为上下文，选区任务不得在范围外落注。六类结果（高亮、替换、删除、下划线、插入文字、自由批注）均须可保存、重开和一次撤销，修订文本不得混入“仅修订文本 / 简短原因 / 详细原因”对应的独立说明。宽松、均衡（默认）、严格三档必须持久化并逐页透传，不得按数量或动作配额凑批注。必须强制前三次可重试模型失败不显示人工决策、第四次失败后才显示重试/跳过/结束；四次请求 ID 均须唯一且只允许一次写回，写回失败不可自动重放，重试中结束后迟到响应不可落注。还应验证实时进度、暂停、继续、结束，以及首次运行复用全文评价的隐私与版权确认。Release validation must cover scope-safe context, all six persisted annotation types, three persistent intensity levels without quotas, one initial model call plus at most three automatic retries with unique IDs, manual decisions only after exhaustion, single writeback, pause/resume/end controls, and the one-time privacy and copyright confirmation.
...
打包脚本会以 `test:release-ui` 对最终可执行文件验证 2.0.11 桌面外壳：关闭临时文档后黄色提示必须消失，两处最近文件列表必须保存并滚动显示 50 项，Logo 对比色必须随主题切换，标题栏工具组在窗口缩放前后都保持几何居中，Windows 最小化、最大化/还原和关闭按钮必须使用可辨识的矢量图标。
