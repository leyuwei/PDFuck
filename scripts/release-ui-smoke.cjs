...
async function main() {
  ...
  try {
    ...
    assert.equal(await confirm.evaluate((element) => element.classList.contains('unsaved-close-confirm')), true, 'close confirmation is missing its danger class')
    assert.equal(await cancel.evaluate((element) => element === document.activeElement), true, 'cancel must receive default focus')
    const dangerBackground = await confirm.evaluate((element) => getComputedStyle(element).backgroundColor)
    assert.ok(['rgb(127, 29, 29)', 'rgba(127, 29, 29, 1)'].includes(dangerBackground), `unexpected close confirmation color: ${dangerBackground}`)
    ...
    await closeEvent
    console.log(JSON.stringify({ releaseUiSmoke: 'passed', version, executable, unsavedClose: { confirmColor: dangerBackground, cancelAnimation: animationName, cancelDefaultFocus: true } }, null, 2))
  } finally {
