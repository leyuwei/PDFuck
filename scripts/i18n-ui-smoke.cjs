...
async function assertAdaptiveToolPanel(page, language, module) {
  ...
  const clippedHints = await page.locator('.tool-panel .tool-panel-action small, .tool-panel .tool-button small').evaluateAll((elements) => elements.flatMap((element) => {
    if (!(element instanceof HTMLElement) || !element.offsetParent) return []
    const style = getComputedStyle(element)
    return style.webkitLineClamp !== 'none' || element.scrollHeight > element.clientHeight + 1
      ? [{ text: element.innerText, lineClamp: style.webkitLineClamp }] : []
  }))
  assert.deepEqual(clippedHints, [], `${language}/${module} tool-card hints must remain fully readable at the chosen UI size`)
}
...
async function main() {
  ...
  try {
    ...
    for (const language of languages) {
      ...
      assert.equal(await page.locator('.ai-timeout-input input').inputValue(), '600', `${language.value} timeout default must be 600 seconds`)
  ...
  try {
    ...
    assert.equal(await page.locator('.edit-tool-icon').count(), 9, 'all nine edit actions must use the shared line-icon system')
