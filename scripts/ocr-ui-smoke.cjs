...
async function main() {
  ...
  try {
    ...
    const run = async range => {
      ...
      const minimizedOcr = page.locator('.ocr-launch')
      assert.equal(await minimizedOcr.getAttribute('data-window-state'), 'minimized', 'minimized OCR must remain visibly marked on its tool button')
      const minimizedColors = await minimizedOcr.evaluate(el => ({ background: getComputedStyle(el).backgroundColor, border: getComputedStyle(el).borderColor }))
      assert.notEqual(minimizedColors.background, 'rgba(0, 0, 0, 0)', `minimized OCR needs a visible background: ${JSON.stringify(minimizedColors)}`)
    ...
    for (const locale of languages) {
      ...
      assert.equal((await page.locator('.about-trigger > span:not([aria-hidden])').innerText()).trim().length > 0, true)
      assert.equal((await page.locator('.about-trigger > small').innerText()).trim(), `v${version}`)
