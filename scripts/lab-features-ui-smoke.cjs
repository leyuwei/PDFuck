...
async function verifyLabFeatures(userData, pdf, requests) {
  ...
  try {
    ...
    await page.mouse.move(collectorHeader.width / 2 + 10, collectorHeader.y + collectorHeader.height / 2, { steps: 6 })
    await page.mouse.up()
    ...
    console.log('[lab-smoke] two context passages recorded')
