...
- **字符级精准选字**：单击 PDF 文字建立光标，按住 `Shift` 并使用左右方向键，即可逐字符扩展或收缩选区。
...
下载 `PDFuck-1.6.0-Windows-Setup.exe`，按安装向导操作即可。安装版支持：
...
下载 `PDFuck-1.6.0-Windows.exe` 后直接运行，不写入固定安装目录，适合放在移动硬盘或临时电脑上使用。
