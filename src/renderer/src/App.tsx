...
import { documentPathKey } from './lib/document-archives'
...
function RecentWelcome({ recent, onOpen, onChoose, onClear }: { recent: RecentPdf[]; onOpen(path: string): void; onChoose(): void; onClear(): void }) {
...
function DocumentTitle({ text, encrypted }: { text: string; encrypted: boolean }) {
  const viewportRef = useRef<HTMLSpanElement>(null)
  const trackRef = useRef<HTMLSpanElement>(null)
  const [overflow, setOverflow] = useState({ active: false, distance: 0, duration: 8 })
  useEffect(() => {
    ...
    const measure = () => {
      ...
      setOverflow((current) => {
        const next = { active: distance > 1, distance, duration: Math.max(7, distance / 22 + 5) }
        return current.active === next.active && Math.abs(current.distance - next.distance) < 1 && Math.abs(current.duration - next.duration) < .1 ? current : next
      })
    }
    measure()
  ...
  return <div className={`document-title${encrypted ? ' encrypted' : ''}${overflow.active ? ' scrolling' : ''}`} title={`${text}${encrypted ? ui("ui.encryptedReadOnly") : ''}`} data-overflowing={overflow.active || undefined} style={{ '--document-title-distance': `${overflow.distance}px`, '--document-title-duration': `${overflow.duration}s` } as CSSProperties}>
    {encrypted && <span className="document-encrypted-badge">{ui("ui.encrypted")}</span>}
    <span ref={viewportRef} className="document-title-viewport"><span ref={trackRef} className="document-title-track">{text}</span></span>
  </div>
}
...
export default function App() {
  ...
  const [ocrDocuments, setOcrDocuments] = useState<number[]>([])
  ...
  const clearRecentFiles = useCallback(async (restoreOpenDialog = false) => {
    const confirmed = await askConfirmation(ui('ui.clearRecentFilesMessage'), { title: ui('ui.clearRecentFilesTitle'), confirmLabel: ui('ui.clearRecentFiles') })
    if (confirmed) {
      try { await window.desktop.clearRecentPdfs(); setRecentFiles([]) }
  ...
  const restoreArchive = useCallback(async (paths: string[]) => {
    ...
    for (const path of paths) {
      if (windowClosingRef.current) { failed.push(path); continue }
      const key = documentPathKey(path, window.desktop.platform)
      if ([...sessionsRef.current.values()].some(session => session.filePath && documentPathKey(session.filePath, window.desktop.platform) === key)) continue
      try { if (!await addOpened(await window.desktop.readPdf(path))) failed.push(path) }
  ...
  const handleReadingPositionChange = useCallback((position: ReadingPosition) => {
    ...
    setActiveBookmarkId((current) => {
      const next = activeBookmarkIdForPosition(liveSessionRef.current.bookmarks, normalized.page, normalized.offset)
  ...
  useEffect(() => {
    setActiveBookmarkId(activeBookmarkIdForPosition(bookmarks, currentPage, readingPositionRef.current.page === currentPage ? readingPositionRef.current.offset : 0))
  }, [activeDocumentId, bookmarks, currentPage])
  ...
  const recognizeDocument = useCallback(async (options: OcrOptions, signal: AbortSignal, progress: (value: OcrProgress) => void) => {
    const model = modelRef.current, documentId = activeDocumentIdRef.current
    if (!model || liveSessionRef.current.encrypted) throw new Error('ocr.invalidRequest')
    return runDocumentOperation(documentId, async () => {
      signal.throwIfAborted()
      const source = await model.ocrSource(options.pages)
      const layers = await recognizePdf(source, options, signal, progress)
      signal.throwIfAborted()
      await model.applyOcrLayers(layers, signal)
      syncModel(layers.length ? t('ocr.done', { count: layers.length }) : ui('ocr.empty'), layers.length > 0, model, documentId)
      return layers.length
    })
  }, [runDocumentOperation, syncModel])
  ...
  const addSelectionAnnotations = useCallback(async (kind: AnnotationKind, value: TextSelection, content = '', color?: string, fallbackPageIndex = currentPage, marks?: TextMark[]) => {
    ...
    await mutate(async (model) => {
      ...
      for (const segment of segments) await model.addAnnotation(segment.pageIndex, kind, segment.rects, content, undefined, color, groupId, preferences.annotationAuthor, undefined, marks)
  ...
  const handleCanvasAction = useCallback(async (action: CanvasAction) => {
    ...
    if (action.tool === 'crop' && action.rect) {
      ...
      await addSelectionAnnotations('highlight', action.selection, annotation.content, annotation.color, action.pageIndex, annotation.marks)
      ...
      await addSelectionAnnotations('replace', action.selection, annotation.content, annotation.color, action.pageIndex, annotation.marks)
  ...
  const editAnnotation = useCallback(async (annotation: AnnotationRecord) => {
    ...
    const model = modelRef.current, documentId = activeDocumentIdRef.current
    const value = await askAnnotation({ kind: annotation.kind, annotationId: annotation.id, initial: annotation.content, marks: annotation.marks, initialColor: annotation.color, reply: annotation.reply, optional: true, edit: true })
    if (value === null || modelRef.current !== model || activeDocumentIdRef.current !== documentId) return
    await mutate((model) => model.updateAnnotationProperties(annotation.id, value.content, value.color, value.reply, value.marks), '批注内容、颜色和回复已更新', false)
  }, [encrypted, mutate])
