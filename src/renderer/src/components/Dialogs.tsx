  ...
  return <div className="modal-backdrop print-modal-backdrop"><div className="modal print-options-dialog"><div className="print-dialog-heading"><div className="print-heading-copy"><span className="print-heading-icon" aria-hidden="true">⎙</span><div><h2>{ui("打印设置与预览")}</h2><p>{t('print.overview', { pages: pages.length, sheets: sheetCount })}</p></div></div><button type="button" aria-label={ui("关闭打印设置")} title={ui("关闭")} onClick={onCancel}>×</button></div>
    <div className="print-dialog-body"><aside className="print-controls">
      ...
      <section className="print-control-section layout-section"><header><b>{ui("页面布局")}</b><span>{multiPage ? t('print.perSheet', { count: rows * columns }) : t('print.onePerSheet')}</span></header>
        <button type="button" role="switch" aria-checked={multiPage} className="print-multipage-toggle" onClick={() => { setMultiPage((value) => !value); setSheetIndex(0) }}><span><b>{ui("合并多页到一张纸")}</b><small>{ui("PDFuck 将按右侧预览直接生成拼版")}</small></span><i aria-hidden="true" /></button>
...
export function ConfirmDialog({ message, title, confirmLabel, destructive = false, onCancel, onConfirm }: { message: string; title?: string; confirmLabel?: string; destructive?: boolean; onCancel(): void; onConfirm(): void }) {
  ...
  return <div className={`modal-backdrop${destructive ? ' unsaved-close-backdrop' : ''}`}><div className={`modal${destructive ? ' unsaved-close-dialog' : ''}`} role="alertdialog" aria-modal="true" aria-labelledby="confirm-dialog-title" aria-describedby="confirm-dialog-message"><h2 id="confirm-dialog-title">{title || (destructive ? ui('未保存的修改') : ui('请确认'))}</h2><p id="confirm-dialog-message">{message}</p><div className="modal-actions"><button ref={cancelRef} type="button" className={destructive ? 'unsaved-close-cancel' : undefined} onClick={onCancel}>{ui('取消')}</button><button type="button" className={destructive ? 'unsaved-close-confirm' : 'primary'} onClick={onConfirm}>{confirmLabel || (destructive ? ui('确认关闭') : ui('确定'))}</button></div></div></div>
}
...
export function UnsavedCloseDialog({ message, title, discardLabel, saveLabel, onDecision }: { message: string; title?: string; discardLabel?: string; saveLabel?: string; onDecision(decision: UnsavedCloseDecision): void }) {
  ...
  return <div className="modal-backdrop unsaved-close-backdrop"><div className="modal unsaved-close-dialog" role="alertdialog" aria-modal="true" aria-labelledby="unsaved-close-title" aria-describedby="unsaved-close-message"><h2 id="unsaved-close-title">{title || ui('未保存的修改')}</h2><p id="unsaved-close-message">{message}</p><div className="modal-actions unsaved-close-actions"><button ref={cancelRef} type="button" className="unsaved-close-cancel" onClick={() => onDecision('cancel')}>{ui('取消')}</button><button type="button" className="unsaved-close-save primary" onClick={() => onDecision('save')}>{saveLabel || ui('保存后关闭')}</button><button type="button" className="unsaved-close-confirm" onClick={() => onDecision('discard')}>{discardLabel || ui('不保存并关闭')}</button></div></div></div>
}
