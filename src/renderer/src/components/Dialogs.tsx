...
export function WatermarkDialog({ initial, existingCount, pageCount, onCancel, onSubmit, onDelete }: { initial?: WatermarkSettings; existingCount: number; pageCount: number; onCancel(): void; onSubmit(value: WatermarkSettings): void; onDelete(): void }) {
  ...
  const previewPage = { width: 600, height: 360 }
  const previewTiles = useMemo(() => watermarkTiles(previewPage, value), [settings.text, settings.size, settings.rotation, settings.density])
  const floating = useFloatingWindow(true)
  ...
  return <div className="modal-backdrop watermark-backdrop" onKeyDownCapture={(event) => { if (event.key === 'Escape') onCancel() }}><ScrollWindow ref={floating.ref} className="modal watermark-dialog" role="dialog" aria-modal="true" aria-labelledby="watermark-title" style={floating.style}>
    ...
    </section><section className="watermark-preview"><h3>{ui('ui.livePreview')}</h3><div>{previewTiles.map((tile, index) => <span key={index} style={{ left: `${(tile.x + tile.width / 2) / previewPage.width * 100}%`, top: `${(tile.y + tile.height / 2) / previewPage.height * 100}%`, color: settings.color, opacity: settings.opacity, fontFamily: fontCssFamily(settings.font), fontSize: `${Math.max(8, Math.min(34, settings.size * .42))}px`, transform: `translate(-50%, -50%) rotate(${settings.rotation}deg)` }}>{settings.text || ui('ui.watermark')}</span>)}</div></section></div>
