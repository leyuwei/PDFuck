...
import type { AnnotationKind, AnnotationReply, PageNumberSettings, TextStyle, WatermarkSettings } from '../types'
...
import { DEFAULT_WATERMARK_SETTINGS, validateWatermarkSettings, watermarkTiles } from '../lib/watermarks'
...
export function OpenPdfDialog({ recent, onCancel, onOpen, onBrowse, onClear }: { recent: RecentPdf[]; onCancel(): void; onOpen(path: string): void; onBrowse(): void; onClear(): void }) {
...
export function WatermarkDialog({ data, currentPage, initial, existingCount, pageCount, onCancel, onSubmit, onDelete }: { data: Uint8Array; currentPage: number; initial?: WatermarkSettings; existingCount: number; pageCount: number; onCancel(): void; onSubmit(value: WatermarkSettings): void; onDelete(): void }) {
  const base = initial || { ...DEFAULT_WATERMARK_SETTINGS, pages: allPageIndices(pageCount) }
  const [settings, setSettings] = useState<WatermarkSettings>(base)
  const [range, setRange] = useState(() => compactPageSelection(base.pages.length ? base.pages : allPageIndices(pageCount)))
  const parsed = useMemo(() => parsePageSelection(range, pageCount), [range, pageCount])
  const value = { ...settings, pages: parsed.pages }
  const invalidRange = parsed.invalid.length > 0 || !parsed.pages.length
  const error = invalidRange ? ui('ui.chooseAValidPageRange') : validateWatermarkSettings(value, pageCount) ? ui('ui.watermarkTextRequired') : undefined
  const { thumbnails, sizes, failed } = usePrintThumbnails(data, [currentPage], 420, 252)
  const previewPage = sizes[currentPage] || { width: 600, height: 800 }
  const previewTiles = useMemo(() => watermarkTiles(previewPage, value), [previewPage.width, previewPage.height, settings.text, settings.size, settings.rotation, settings.density])
  const floating = useFloatingWindow(true)
  const update = <K extends keyof WatermarkSettings,>(key: K, next: WatermarkSettings[K]) => setSettings((current) => ({ ...current, [key]: next }))
  return <div className="modal-backdrop watermark-backdrop" onKeyDownCapture={(event) => { if (event.key === 'Escape') onCancel() }}><ScrollWindow ref={floating.ref} className="modal watermark-dialog" role="dialog" aria-modal="true" aria-labelledby="watermark-title" style={floating.style}>
    <header className="watermark-heading" {...floating.dragHandlers}><span aria-hidden="true">W</span><div><h2 id="watermark-title">{ui('ui.addWatermark')}</h2><p>{ui('ui.watermarkDescription')}</p></div><button type="button" aria-label={ui('ui.close')} title={ui('ui.close')} onClick={onCancel}>×</button></header>
    {existingCount > 0 && <div className="page-number-existing"><b>{ui('ui.existingWatermarksFound')}</b><span>{t('watermark.existing', { count: existingCount })}</span></div>}
    <div className="watermark-layout"><section className="watermark-fields">
      <label className={invalidRange ? 'invalid' : ''}><span>{ui('ui.pageRange')}</span><input autoFocus dir="ltr" value={range} aria-invalid={invalidRange} placeholder={ui('ui.forExample135810')} onChange={(event) => setRange(event.target.value)} /><small>{parsed.invalid.length ? t('page.rangeInvalid', { value: parsed.invalid.join(', ') }) : t('page.selected', { count: parsed.pages.length })}</small></label>
      <label><span>{ui('ui.watermarkText')}</span><input value={settings.text} maxLength={120} onChange={(event) => update('text', event.target.value)} /></label>
      <div className="watermark-format"><label><span>{ui('ui.font')}</span><select value={normalizeFontFamily(settings.font)} onChange={(event) => update('font', event.target.value)}>{fontOptionsFor(settings.font).map((option) => <option key={option.value} value={option.value}>{localizedFontLabel(option.label)}</option>)}</select></label><label><span>{ui('ui.fontSize')}</span><input type="number" min="8" max="144" value={settings.size} onChange={(event) => update('size', Math.max(8, Math.min(144, Number(event.target.value) || 8)))} /></label><label><span>{ui('ui.color')}</span><input type="color" value={settings.color} onChange={(event) => update('color', event.target.value)} /></label></div>
      <div className="watermark-format"><label><span>{ui('ui.rotationAngle')}</span><div className="input-suffix"><input type="number" min="-180" max="180" value={settings.rotation} onChange={(event) => update('rotation', Math.max(-180, Math.min(180, Number(event.target.value) || 0)))} /><i>°</i></div></label><label><span>{ui('ui.opacity')}</span><div className="input-suffix"><input type="number" min="5" max="100" value={Math.round(settings.opacity * 100)} onChange={(event) => update('opacity', Math.max(.05, Math.min(1, (Number(event.target.value) || 5) / 100)))} /><i>%</i></div></label></div>
      <label className="watermark-density"><span>{ui('ui.watermarkDensity')}</span><input type="range" min="1" max="5" step="1" value={settings.density} onChange={(event) => update('density', Number(event.target.value))} /><small><span>{ui('ui.sparse')}</span><span>{ui('ui.balanced')}</span><span>{ui('ui.dense')}</span></small></label>
    </section><section className="watermark-preview"><h3>{ui('ui.livePreview')}</h3><div className="watermark-preview-stage"><svg className="watermark-preview-page" role="img" aria-label={ui('ui.livePreview')} width={Math.round(252 * previewPage.width / previewPage.height)} viewBox={`0 0 ${previewPage.width} ${previewPage.height}`}><rect width="100%" height="100%" fill="#fff" />{thumbnails[currentPage] && <image href={thumbnails[currentPage]} width={previewPage.width} height={previewPage.height} />}{previewTiles.map((tile, index) => <text key={index} x={tile.x + tile.width / 2} y={tile.y + tile.height / 2} fill={settings.color} fillOpacity={settings.opacity} fontFamily={fontCssFamily(settings.font)} fontSize={settings.size} textAnchor="middle" dominantBaseline="central" transform={`rotate(${settings.rotation} ${tile.x + tile.width / 2} ${tile.y + tile.height / 2})`}>{settings.text || ui('ui.watermark')}</text>)}</svg>{!thumbnails[currentPage] && <small>{failed.has(currentPage) ? ui('ui.previewUnavailable') : ui('ui.generatingPreview')}</small>}</div></section></div>
    <div className="modal-actions page-number-actions">{existingCount > 0 && <button type="button" className="danger" onClick={onDelete}>{ui('ui.removeExistingWatermarks')}</button>}<span /><button type="button" onClick={onCancel}>{ui('ui.cancel')}</button><button type="button" className="primary" disabled={Boolean(error)} title={error} onClick={() => onSubmit(value)}>{existingCount > 0 ? ui('ui.updateWatermarks') : ui('ui.applyWatermark')}</button></div>
  </ScrollWindow></div>
}
