...
describe('ToolPanel theme colours', () => {
  ...
  it('uses the same action-card system for edit and save tools', async () => {
    ...
    expect(container.querySelector('.ocr-launch')?.getAttribute('data-window-state')).toBe('closed')
    await act(async () => root.render(<ToolPanel {...common} module="edit" ocrWindowState="minimized" />))
    expect(container.querySelector('.ocr-launch')?.getAttribute('data-window-state')).toBe('minimized')
    await act(async () => root.render(<ToolPanel {...common} module="edit" ocrWindowState="open" />))
    expect(container.querySelector('.ocr-launch')?.getAttribute('data-window-state')).toBe('open')
