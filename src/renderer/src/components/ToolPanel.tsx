...
export function ToolPanel(props: Props) {
  ...
  const interfaceSize = useInterfaceSize()
  const [exportDpiInput, setExportDpiInput] = useState(() => String(props.exportDpi))
  const [shapeCreatorDocumentId, setShapeCreatorDocumentId] = useState<number | null>(null)
  const documentSessionKey = props.documentSessionKey ?? 0
  ...
  useEffect(() => { setShapeCreatorDocumentId(null) }, [documentSessionKey])
  ...
  return <><aside className="tool-panel"><ScrollWindow className="tool-panel-section" autoHideScrollbar><h2>{ui(({ view: "ui.view", edit: "ui.edit", annotate: "ui.annotate", save: "ui.save" } as const)[module])}</h2>
    {module === 'view' && <><p className="subtitle">{ui("ui.chooseAPageLayoutForYourReadingFlow")}</p><h3>{ui("ui.pageLayout")}</h3>
...
<button type="button" className="wide tool-action-button interface-size-action" onClick={() => setSizeDialogOpen(true)}><span>{ui("ui.interfaceFontSize")}</span><output>{ui(INTERFACE_SIZE_LABELS[INTERFACE_SIZES.findIndex(size => size === interfaceSize)])}</output></button>
<h3>{ui("ui.interfaceLanguage")}</h3><label className="language-select"><select value={language} aria-label={ui("ui.interfaceLanguage")} onChange={(event) => setInterfaceLanguage(event.target.value as InterfaceLanguage)}>{INTERFACE_LANGUAGES.map((code) => <option key={code} value={code}>{LANGUAGE_LABELS[code]}</option>)}</select></label>
      <h3>{ui("ui.readingTools")}</h3><div className="reading-tools"><button type="button" className="wide tool-action-button search-pdf-action" disabled={!hasDocument} onClick={props.onSearch}><span>{ui("ui.searchPdf")}</span><kbd>{shortcutLabel('search', platform)}</kbd></button><button className="wide tool-action-button recognize-bookmark-action" disabled={!hasDocument || readOnly} onClick={() => props.onRecognizeBookmarks?.()}>{ui("ui.recognizeBookmarks")}</button><button className="wide tool-action-button" disabled={!hasDocument} onClick={props.onVisuals}>{ui("ui.findFiguresTables")}</button><button className={`wide tool-action-button${props.citationsEnabled ? ' active' : ''}`} disabled={!hasDocument} aria-pressed={props.citationsEnabled} onClick={props.onCitations}>{props.citationsEnabled ? ui("ui.hideCitationLinks") : ui("ui.linkCitations")}</button><button className="wide tool-action-button" disabled={!hasDocument} onClick={props.onGrammar}>{ui("ui.grammarCheck")}</button></div>
      ...
      <div className="info-card"><b>{ui("ui.readingTip")}</b><span>{t('shortcut.navigationHint', { zoom: mac ? '⌘' : 'Ctrl', page: mac ? 'Option' : 'Alt' })}</span></div></>}
    {module === 'edit' && <><p className="subtitle">{ui("ui.adjustPagesDirectlyOrAddFormattedTextAndImages")}</p><h3>{ui("ui.page2")}</h3>
