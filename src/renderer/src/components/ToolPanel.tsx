...
export function ToolPanel(props: Props) {
  ...
  return <><aside className="tool-panel">
    {module === 'view' && <ScrollWindow className="tool-panel-section"><h2>{ui("ui.view")}</h2><p className="subtitle">{ui("ui.chooseAPageLayoutForYourReadingFlow")}</p><h3>{ui("ui.pageLayout")}</h3>
...
<h3>{ui("ui.interfaceLanguage")}</h3><label className="language-select"><select value={language} aria-label={ui("ui.interfaceLanguage")} onChange={(event) => setInterfaceLanguage(event.target.value as InterfaceLanguage)}>{INTERFACE_LANGUAGES.map((code) => <option key={code} value={code}>{LANGUAGE_LABELS[code]}</option>)}</select></label>
      ...
      <div className="info-card"><b>{ui("ui.readingTip")}</b><span>{t('shortcut.navigationHint', { zoom: mac ? '⌘' : 'Ctrl', page: mac ? 'Option' : 'Alt' })}</span></div></ScrollWindow>}
    {module === 'edit' && <ScrollWindow className="tool-panel-section"><h2>{ui("ui.edit")}</h2><p className="subtitle">{ui("ui.adjustPagesDirectlyOrAddFormattedTextAndImages")}</p><h3>{ui("ui.page2")}</h3>
      ...
      <PanelAction disabled={documentDisabled} icon={<EditIcon kind="page_numbers" />} onClick={() => props.onPageNumbers?.()} hint={ui("ui.addCustomizableRemovablePageNumbersToEveryPage")}>{ui("ui.addPageNumbers")}</PanelAction></ScrollWindow>}
    {module === 'annotate' && <ScrollWindow className="tool-panel-section"><h2>{ui("ui.annotate")}</h2><p className="subtitle">{t('shortcut.annotationSelectionHint', { add: mac ? '⌘' : 'Ctrl', remove: shortcutLabel('deleteSelection', platform) || '' })}</p><h3>{ui("ui.textAnnotations")}</h3>
      ...
      <ToolButton tool="insert" activeTool={activeTool} onTool={onTool} disabled={documentDisabled} icon={<AnnotationIcon kind="insert" />} shortcut={shortcutLabel('insert', platform)} hint={ui("ui.clickATextGapToInsert")}>{ui("ui.insertText")}</ToolButton></ScrollWindow>}
    {module === 'save' && <ScrollWindow className="tool-panel-section"><h2>{ui("ui.save")}</h2><p className="subtitle">{ui("ui.saveTheCompleteDocumentOrPrintAndExportOnlyThe")}</p><h3>PDF</h3>
