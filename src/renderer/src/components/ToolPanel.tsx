...
<h3>{ui('界面语言', 'Interface Language')}</h3><label className="language-select"><span aria-hidden="true">◎</span><select value={language} aria-label={ui('界面语言', 'Interface Language')} onChange={(event) => setInterfaceLanguage(event.target.value as typeof language)}><option value="zh">简体中文</option><option value="en">English</option><option value="ja">日本語</option><option value="ru">Русский</option><option value="es">Español</option></select></label>
    ...
    {module === 'edit' && <section><h2>{ui('编辑', 'Edit')}</h2><p className="subtitle">{ui('直接调整页面或添加带格式的文字内容。', 'Adjust pages directly or add formatted text.')}</p><h3>{ui('页面', 'Page')}</h3>
      ...
      <button className="danger wide" disabled={disabled} onClick={props.onDeletePages}>{ui('批量删除页面…', 'Delete Pages…')}</button><h3>{ui('内容', 'Content')}</h3>
    ...
    {module === 'save' && <section><h2>{ui('保存', 'Save')}</h2><p className="subtitle">{ui('保存完整文档，或只打印、导出真正需要的页面。', 'Save the complete document, or print and export only the pages you need.')}</p><h3>PDF</h3>
      <button className="primary wide" disabled={disabled} onClick={() => props.onSave(false)}>{ui('保存 PDF', 'Save PDF')}</button><button className="wide" disabled={disabled} onClick={() => props.onSave(true)}>{ui('另存为 PDF…', 'Save As PDF…')}</button><h3>{ui('打印', 'Print')}</h3>
      <button className="wide print-button" disabled={disabled || props.printing} onClick={props.onPrint}><span>{props.printing ? ui('正在打开打印对话框…', 'Opening print dialog…') : ui('选择页面并打印…', 'Select Pages & Print…')}</span><kbd>Ctrl+P</kbd></button><p className="hint">{ui('可选择连续或不连续页码，包含尚未保存的修改。', 'Choose continuous or non-contiguous pages, including unsaved changes.')}</p><h3>{ui('指定页面导出', 'Export Selected Pages')}</h3>
      <label>{ui('文件格式', 'File Format')}<select value={props.exportFormat} onChange={(event) => props.onExportFormat(event.target.value as Props['exportFormat'])}><option value="pdf">PDF</option><option value="png">PNG</option><option value="jpg">JPG</option><option value="eps">EPS</option></select></label>
      ...
      {props.exportFormat !== 'pdf' && <label>{ui('输出清晰度', 'Output Resolution')}<div className="input-suffix"><input type="number" min="72" max="600" value={props.exportDpi} onChange={(event) => props.onExportDpi(Math.max(72, Math.min(600, Number(event.target.value))))} /><span>DPI</span></div></label>}
      <button className="wide" disabled={disabled} onClick={props.onExport}>{ui('选择页面并导出…', 'Select Pages & Export…')}</button><p className="hint">{ui('可选不连续页码；文件名保留原文档页码后缀。', 'Non-contiguous pages are supported; filenames keep the original page suffix.')}</p></section>}
  </aside>
}
