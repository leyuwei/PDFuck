      ...
      for (let issueIndex = 0; issueIndex < issueTypes.length; issueIndex += 1) {
        ...
        for (let index = 0; index < targetPages.length;) {
          ...
          while (!response && !skipPage) {
            ...
            try {
              response = await autoAnnotatePage(settingsSnapshot, { pageIndex: page.pageIndex, blocks: page.blocks, issueType, customIssue: customIssueSnapshot, opening, previous, next, contextSummary, detail: detailSnapshot, intensity: intensitySnapshot, retryAttempt: retries, language: interfaceLanguage }, requestId)
            } catch (cause) {
              if (automaticRunId.current !== runId) return
