...
export const AUTOMATIC_ANNOTATION_CUSTOM_KEY = 'pdfuck.lab.auto-annotation-custom.v1'
...
const AUTOMATIC_ISSUE_LABELS: Record<AutomaticAnnotationIssueType, TranslationKey> = {
  ...
  section_structure: 'ui.autoIssueSectionStructure', restructuring: 'ui.autoIssueRestructuring', academic_contribution: 'ui.autoIssueAcademicContribution', custom: 'ui.autoIssueCustom'
}

export interface AnnotationSuggestionRequest extends AutomaticAnnotationContextRequest { token: number; annotationId: string; annotationContent: string; pageIndex: number; embedded?: boolean }
...
function loadAutomaticAnnotationIssues(): AutomaticAnnotationIssueType[] {
  try {
    ...
    return Array.isArray(value) ? AUTOMATIC_ANNOTATION_ISSUE_TYPES.filter((issue) => value.includes(issue)) : AUTOMATIC_ANNOTATION_ISSUE_TYPES.filter((issue) => issue !== 'custom')
  } catch { return AUTOMATIC_ANNOTATION_ISSUE_TYPES.filter((issue) => issue !== 'custom') }
}
...
function summaryText(value: string, limit = 100): string { const clipped = clipStart(value.trim(), limit); return clipped + (characters(value.trim()).length > limit ? '…' : '') }
...
interface AiActivityRequest { model: string; details: string[] }
type AiActivityState = AiStreamProgress & { request?: AiActivityRequest }

const emptyAiActivity = (request?: AiActivityRequest): AiActivityState => ({ reasoning: '', output: '', received: false, truncated: false, request })

function AiActivity({ progress, busy }: { progress: AiActivityState; busy: boolean }) {
  if (!busy && !progress.received && !progress.recovery) return null
  const status = !busy && !progress.received ? ui('ui.aiRecoveryStopped') : !busy && progress.received ? ui('ui.modelResponseEnded')
    : !progress.request ? ui('ui.preparingModelRequest')
      : !progress.received ? (progress.recovery ? ui('ui.aiRecovering') : ui('ui.waitingForFirstModelOutput'))
    : progress.reasoning && !progress.output ? ui('ui.modelIsThinking')
      : ui('ui.modelIsGenerating')
...
export function AnnotationLab({ visible = true, selection, selectionKey, documentKey, platform = 'win32', disabled = false, annotationSuggestionsEnabled = false, suggestionRequest, suggestionEditor, onSuggestionRequestConsumed, onAnnotationSuggestionsEnabledChange, getDocument, getAutomaticContext, getAutomaticAnnotationPages, onAdd, onAddFullReview, onAddSuggestion, onAddAutomaticAnnotations, onAddDrawing, onExportDrawing, onCopy }: Props) {
  ...
  const floating = useFloatingWindow(visible && !(activeWindow === 'suggestion' && suggestionEditor) ? activeWindow : undefined)
  const openWindow = (kind: LabWindow) => {
    setOpenedWindows((current) => current.includes(kind) ? current : [...current, kind])
    setActiveWindow(kind)
  }
  const closeWindow = () => {
  ...
  const [polishActivity, setPolishActivity] = useState<AiActivityState>(emptyAiActivity)
  ...
  const [reviewActivity, setReviewActivity] = useState<AiActivityState>(emptyAiActivity)
  ...
  const [suggestionActivity, setSuggestionActivity] = useState<AiActivityState>(emptyAiActivity)
  ...
  const [customIssue, setCustomIssue] = useState(() => (localStorage.getItem(AUTOMATIC_ANNOTATION_CUSTOM_KEY) || '').slice(0, 4000))
  ...
  const [automaticActivity, setAutomaticActivity] = useState<AiActivityState>(emptyAiActivity)
  ...
  const reviewProgress = reviewStartedAt === undefined ? undefined : aiRequestProgress(aiRecoveryTimeoutSeconds(settings.timeoutSeconds), reviewStartedAt, reviewProgressNow)
  ...
  useEffect(() => {
    ...
    const handler = (event: KeyboardEvent) => {
      if (!isImeCompositionKey(event) && !isTextEntryEvent(event) && (event.ctrlKey || event.metaKey) && event.key.toLowerCase() === 'i' && normalizedSelection) { event.preventDefault(); openWindow('polish') }
    }
  ...
  useEffect(() => {
    if (!visible) return
    const handler = () => { if (normalizedSelection) openWindow('polish') }
  ...
  useEffect(() => {
    ...
    setResult(''); setError(''); setBusy(false); setAdding(false); setPolishActivity(emptyAiActivity())
  ...
  useEffect(() => {
    ...
    openWindow('suggestion')
    onSuggestionRequestConsumed?.(suggestionRequest.token)
  }, [suggestionRequest?.token, documentKey, onSuggestionRequestConsumed])
  useEffect(() => {
    if (!activeSuggestionRequest?.embedded || suggestionEditor) return
    suggestionRequestId.current += 1
    cancelAiRequest(suggestionActivity.requestId)
    setActiveSuggestionRequest(undefined); setSuggestionBusy(false); setSuggestionResult('')
    setOpenedWindows(current => current.filter(kind => kind !== 'suggestion'))
    setActiveWindow(current => current === 'suggestion' ? undefined : current)
  }, [suggestionEditor, activeSuggestionRequest, suggestionActivity.requestId])
  useEffect(() => {
    let active = true
  ...
  useEffect(() => () => cancelAiRequest(polishActivity.requestId), [polishActivity.requestId])
  useEffect(() => () => cancelAiRequest(reviewActivity.requestId), [reviewActivity.requestId])
  useEffect(() => () => cancelAiRequest(suggestionActivity.requestId), [suggestionActivity.requestId])
  useEffect(() => () => {
  ...
  const submitPolish = async () => {
    ...
    const request = { model: settings.model, details: [message('lab.aiSelectionInput', { count: characters(normalizedSelection).length }), `${t('ui.polishingInstruction')} · ${summaryText(instruction)}`] }
    setBusy(true); setError(''); setResult(''); setPolishActivity(emptyAiActivity(request))
    try { const output = await polishText(settings, instruction, normalizedSelection, (progress) => { if (polishRequestId.current === current) setPolishActivity((activity) => ({ ...progress, request: activity.request })) }); if (polishRequestId.current === current) setResult(output) }
  ...
  const addPolishResult = async () => {
    ...
    try { await onAdd(result); setAdding(false); closeWindow() } catch (cause) { setError(cause instanceof Error ? cause.message : String(cause)); setAdding(false) }
  }
  const submitReview = async () => {
    ...
    setReviewBusy(true); setReviewError(''); setReviewResult(''); setReviewActivity(emptyAiActivity())
    try {
      const document = await getDocument(reviewMode)
      const input = reviewMode === 'text'
        ? message('lab.aiTextDocumentInput', { name: document.name, count: characters(document.text || '').length })
        : message('lab.aiPdfDocumentInput', { name: document.name, size: Math.max(0.01, document.bytes.length / 1024 / 1024).toFixed(2) })
      setReviewActivity(emptyAiActivity({ model: settings.model, details: [input, `${t('ui.reviewPrompt')} · ${summaryText(reviewInstruction)}`] }))
      const output = await reviewDocument(settings, reviewInstruction, document, reviewMode, interfaceLanguage, (progress) => { if (reviewRequestId.current === current) setReviewActivity((activity) => ({ ...progress, request: activity.request })) })
  ...
  const addReviewResult = async () => {
    ...
    try { await onAddFullReview(reviewResult); setReviewAdding(false); closeWindow() } catch (cause) { setReviewError(cause instanceof Error ? cause.message : String(cause)); setReviewAdding(false) }
  }
  ...
  const startAutomaticAnnotation = async () => {
    ...
    const customIssueSnapshot = customIssue.trim()
    if (issueTypes.includes('custom') && !customIssueSnapshot) { setAutomaticError(t("ui.customIssueRequired")); return }
    ...
    setAutomaticError(''); setAutomaticStatus('extracting'); setAutomaticActivity(emptyAiActivity())
      ...
      for (let issueIndex = 0; issueIndex < issueTypes.length; issueIndex += 1) {
        ...
        for (let index = 0; index < targetPages.length;) {
          ...
          while (!response && !skipPage) {
            ...
            const contextCount = [opening, previous, next, contextSummary].filter(Boolean).length
            setAutomaticActivity(emptyAiActivity({ model: settingsSnapshot.model, details: [message('lab.aiAutomaticInput', { page: page.pageIndex + 1, blocks: page.blocks.length, issue: t(AUTOMATIC_ISSUE_LABELS[issueType]), contexts: contextCount })] }))
            try {
              response = await autoAnnotatePage(settingsSnapshot, { pageIndex: page.pageIndex, blocks: page.blocks, issueType, customIssue: customIssueSnapshot, opening, previous, next, contextSummary, detail: detailSnapshot, intensity: intensitySnapshot, language: interfaceLanguage }, requestId, (progress) => { if (automaticRunId.current === runId && automaticAiRequestId.current === requestId) setAutomaticActivity((activity) => ({ ...progress, request: activity.request })) })
            } catch (cause) {
              if (automaticRunId.current !== runId) return
  ...
  const submitSuggestion = async () => {
    ...
    const request = { model: settings.model, details: [message('lab.aiSuggestionInput', { contexts: requestContexts.length }), `${t('ui.suggestionPrompt')} · ${summaryText(suggestionInstruction)}`] }
    setSuggestionBusy(true); setSuggestionError(''); setSuggestionResult(''); setSuggestionActivity(emptyAiActivity(request))
    try {
      const output = await suggestForAnnotation(settings, suggestionInstruction, activeSuggestionRequest.annotationContent, requestContexts, interfaceLanguage, (progress) => { if (suggestionRequestId.current === current) setSuggestionActivity((activity) => ({ ...progress, request: activity.request })) })
  ...
  const addSuggestionResult = async () => {
    if (!suggestionResult || suggestionAdding || !activeSuggestionRequest || (activeSuggestionRequest.embedded ? !suggestionEditor : !onAddSuggestion)) return
    setSuggestionAdding(true); setSuggestionError('')
    try { if (activeSuggestionRequest.embedded) { if (!suggestionEditor) return; suggestionEditor.apply(suggestionResult) } else await onAddSuggestion?.(activeSuggestionRequest.annotationId, suggestionResult); setSuggestionAdding(false); setOpenedWindows(current => current.filter(kind => kind !== 'suggestion')); setActiveWindow(current => current === 'suggestion' ? undefined : current); setActiveSuggestionRequest(undefined) } catch (cause) { setSuggestionError(cause instanceof Error ? cause.message : String(cause)); setSuggestionAdding(false) }
  }

  const windowStyle = floating.style
  ...
  return <div className="annotation-lab" hidden={!visible}>
    ...
    {activeWindow === 'automatic' && <ScrollWindow className="ai-polish-window lab-workflow-window automatic-annotation-window" ref={floating.ref} style={windowStyle}><header {...floating.dragHandlers}><span><AnnotationIcon kind="ai_annotate" />{t("ui.automaticAnnotation")}</span><span className="lab-window-actions"><button type="button" onClick={() => setActiveWindow(undefined)} aria-label={t("ui.minimizeLabWindow")} title={t("ui.minimizeLabWindow")}>−</button><button type="button" onClick={closeWindow} aria-label={t("ui.close")}>×</button></span></header><p className="lab-workflow-intro">{t("ui.automaticAnnotationIntro")}</p>
      ...
      {!automaticActive && <button type="button" className="primary wide automatic-start" disabled={!getAutomaticAnnotationPages || !onAddAutomaticAnnotations || !automaticIssues.length || (automaticIssues.includes('custom') && !customIssue.trim())} onClick={() => void startAutomaticAnnotation()}>{automaticIssues.length ? t("ui.startAutomaticAnnotation") : t("ui.selectAtLeastOneIssueType")}</button>}
      ...
      <AiActivity progress={automaticActivity} busy={automaticStatus === 'running' || automaticStatus === 'pausing'} />
    ...
    {activeWindow === 'suggestion' && activeSuggestionRequest && !activeSuggestionRequest.embedded && <ScrollWindow className="ai-polish-window lab-workflow-window annotation-suggestion-window" ref={floating.ref} style={windowStyle}><header {...floating.dragHandlers}><span><AnnotationIcon kind="ai_suggest" />{t("ui.annotationSuggestions")}</span><span className="lab-window-actions"><button type="button" onClick={() => setActiveWindow(undefined)} aria-label={t("ui.minimizeLabWindow")} title={t("ui.minimizeLabWindow")}>−</button><button type="button" onClick={() => { closeWindow(); setActiveSuggestionRequest(undefined) }} aria-label={t("ui.close")}>×</button></span></header>{suggestionContent}</ScrollWindow>}

    </>}
    {activeSuggestionRequest?.embedded && suggestionEditor && createPortal(<div className="lab-workflow-window annotation-suggestion-window annotation-suggestion-inline" onKeyDownCapture={(event) => { if (event.key === 'Escape' && !event.nativeEvent.isComposing) { event.preventDefault(); event.stopPropagation(); suggestionEditor.close() } }}>{suggestionContent}</div>, suggestionEditor.element)}
    {openedWindows.includes('drawing') && onAddDrawing && onExportDrawing && <DrawingBoard hidden={!visible || activeWindow !== 'drawing'} onMinimize={() => setActiveWindow(undefined)} minimizeLabel={t("ui.minimizeLabWindow")} labels={{ title: t("ui.freeDrawingBoard"), description: t("ui.drawFreelyOnAResizableCanvasThenExportOrAddToCurrentPage"), brushSize: t("ui.brushSize"), color: t("ui.color"), canvasActions: t("ui.canvasActions"), clearCanvas: t("ui.clearCanvas"), drawingArea: t("ui.drawingArea"), moveResizeHint: t("ui.dragTheTitleBarToMoveTheBoardAndACornerToResizeIt"), startDrawingHere: t("ui.startDrawingHere"), drawingHint: t("ui.pressAndDragWithAMouseOrStylusToDraw"), exportPng: t("ui.exportPng"), addToPage: t("ui.addToCurrentPage"), close: t("ui.close"), encodingFailed: t("ui.unableToEncodeDrawing"), actionFailed: t("ui.actionFailed") }} onClose={closeWindow} onAddPng={onAddDrawing} onExportPng={onExportDrawing} />}
  </div>
}
