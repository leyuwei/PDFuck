...
type LabWindow = 'polish' | 'review' | 'suggestion' | 'drawing'
...
export function AnnotationLab({ visible = true, selection, selectionKey, documentKey, platform = 'win32', disabled = false, annotationSuggestionsEnabled = false, suggestionRequest, onSuggestionRequestConsumed, onAnnotationSuggestionsEnabledChange, getDocument, getAutomaticContext, onAdd, onAddFullReview, onAddSuggestion, onAddDrawing, onExportDrawing, onCopy }: Props) {
  ...
  return <div className="annotation-lab">
    ...
    {activeWindow === 'drawing' && onAddDrawing && onExportDrawing && <DrawingBoard labels={{ title: t("ui.freeDrawingBoard"), brushSize: t("ui.brushSize"), color: t("ui.color"), clear: t("ui.clear"), drawingArea: t("ui.drawingArea"), exportPng: t("ui.exportPng"), addToPage: t("ui.addToCurrentPage"), close: t("ui.close"), encodingFailed: t("ui.unableToEncodeDrawing"), actionFailed: t("ui.actionFailed") }} onClose={() => setActiveWindow(undefined)} onAddPng={onAddDrawing} onExportPng={onExportDrawing} />}
  </div>
}
