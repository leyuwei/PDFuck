...
function AiActivity({ progress, busy }: { progress: AiActivityState; busy: boolean }) {
  if (!busy && !progress.received) return null
  const status = !busy && progress.received ? ui('ui.modelResponseEnded')
    : !progress.request ? ui('ui.preparingModelRequest')
      : !progress.received ? ui('ui.waitingForFirstModelOutput')
...
export function AnnotationLab({ visible = true, selection, selectionKey, documentKey, platform = 'win32', disabled = false, annotationSuggestionsEnabled = false, suggestionRequest, onSuggestionRequestConsumed, onAnnotationSuggestionsEnabledChange, getDocument, getAutomaticContext, getAutomaticAnnotationPages, onAdd, onAddFullReview, onAddSuggestion, onAddAutomaticAnnotations, onAddDrawing, onExportDrawing, onCopy }: Props) {
  ...
  const startAutomaticAnnotation = async () => {
    ...
    setAutomaticError(''); setAutomaticRetryAttempt(0); setAutomaticStatus('extracting'); setAutomaticActivity(emptyAiActivity())
  ...
  return <div className="annotation-lab" hidden={!visible}>
    ...
    {settingsOpen && <div className="annotation-lab-settings-backdrop" role="presentation" onPointerDown={() => setSettingsOpen(false)}><section className="annotation-lab-settings" role="dialog" aria-modal="true" aria-label={t("ui.labModelSettings")} onPointerDown={(event) => event.stopPropagation()}><header><b>{t("ui.labModelSettings")}</b><button type="button" aria-label={t("ui.minimizeLabWindow")} onClick={() => { setSettingsMinimized(true); setSettingsOpen(false) }}>−</button><button type="button" onClick={() => setSettingsOpen(false)} aria-label={t("ui.closeModelSettings")}>×</button></header>
