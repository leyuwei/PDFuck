...
describe('annotation localization', () => {
  ...
  it('localizes blank annotation defaults, quick replies, and the delete action', async () => {
    ...
    await act(async () => root.render(<AnnotationPanel collapsed={false} annotationAuthor="PDFuck" showAnnotationAuthors={false} theme="light" accent="#5575de" annotations={[{ id: 'empty', pageIndex: 0, kind: 'underline', author: '', content: '', color: '#000000', reply: { status: 'handled', content: '已处理' }, rects: [] }]} onAuthorSettings={() => undefined} onToggle={() => undefined} onSelect={() => undefined} onEdit={async () => undefined} onReply={async () => undefined} onDelete={() => undefined} />))
    ...
    expect(container.querySelector('.annotation-status-button.handled')?.getAttribute('title')).toBe('Resolved')
    await act(async () => root.unmount())
  })
})
