    ...
    {open && <div className="document-archives-popover" role="dialog" aria-labelledby="document-archives-title" aria-busy={busy}>
      ...
      <div className="document-archives-body">
        ...
        <div className="archive-list">{archives.length ? archives.map(archive => <article key={archive.id}>
          <button type="button" className="archive-restore" disabled={busy} onClick={() => void restore(archive)} title={ui('archives.restore')}><bdi>{archive.name}</bdi><small>{t('archives.files', { count: archive.paths.length })}</small></button>
          {deleting !== archive.id && <div className="archive-actions"><button type="button" className="archive-action-button" disabled={busy} aria-label={ui('archives.rename')} title={ui('archives.rename')} onClick={() => { setEditing(archive.id); setName(archive.name); input.current?.focus() }}><svg viewBox="0 0 20 20" aria-hidden="true"><path d="m4 14 1-4 7-7 3 3-7 7-4 1Zm7-10 3 3" /></svg></button><button type="button" className="archive-action-button danger" disabled={busy} aria-label={ui('archives.delete')} title={ui('archives.delete')} onClick={() => setDeleting(archive.id)}><svg viewBox="0 0 20 20" aria-hidden="true"><path d="M4 6h12M8 3h4l1 3H7l1-3Zm-2 3 1 11h6l1-11M9 9v5m2-5v5" /></svg></button></div>}
          {deleting === archive.id && <div className="archive-delete-confirm"><span>{ui('archives.deleteConfirm')}</span><div><button type="button" className="danger" disabled={busy} onClick={() => { try { setArchives(deleteDocumentArchive(archive.id)); setDeleting(undefined); if (editing === archive.id) { setEditing(undefined); setName('') } } catch { setError(true) } }}>{ui('archives.delete')}</button><button type="button" onClick={() => setDeleting(undefined)}>{ui('ui.cancel')}</button></div></div>}
