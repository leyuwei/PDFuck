...
export function AnnotationPanel({ annotations, selectedId, selectedIds = [], collapsed, onToggle, onSelect, onEdit, onColor, onReply, onDelete }: Props) {
  ...
  return <aside className={`annotation-panel${singleLine ? ' single-line' : ''}`} style={{ '--annotation-list-font-size': `${fontSize}px`, width: panelWidth, flexBasis: panelWidth } as CSSProperties}><div className="annotation-resize-handle" role="separator" aria-label="调整批注列表宽度" title="拖动调整批注列表宽度" onPointerDown={beginResize} onPointerMove={moveResize} onPointerUp={finishResize} /><div className="annotation-heading"><div><h2>批注列表</h2><p>{annotations.length} 条批注{selectedIds.length ? ` · 已选 ${selectedIds.length} 条` : ''}</p></div><div className="annotation-heading-actions"><button type="button" className="annotation-collapse" onClick={onToggle} title="收起批注列表" aria-label="收起批注列表"><span>›</span> 收起</button></div></div>
    ...
    <div className="annotation-list">{annotations.length ? annotations.map((annotation) => <AnnotationRow key={annotation.id} annotation={annotation} selected={selectedIds.includes(annotation.id) || annotation.id === selectedId} onSelect={(options) => onSelect(annotation, options)} onEdit={(content) => onEdit(annotation.id, content)} onColor={(color) => onColor(annotation.id, color)} onReply={(reply) => onReply(annotation.id, reply)} />) : <div className="empty-list">还没有批注<br /><small>在页面上框选文字开始批注</small></div>}</div>
    <div className="annotation-actions"><button onClick={() => onDelete(selectedIds.length ? selectedIds : selected ? [selected.id] : [])} disabled={!selectedIds.length && !selected} className="danger">删除{selectedIds.length > 1 ? ` ${selectedIds.length} 条` : ''}批注</button></div>
  </aside>
}
