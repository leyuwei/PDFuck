...
export function AnnotationPanel({ annotations, selectedId, selectedIds = [], collapsed, onToggle, onSelect, onEdit, onColor, onReply, onDelete }: Props) {
  ...
  return <aside className={`annotation-panel${singleLine ? ' single-line' : ''}`} style={{ '--annotation-list-font-size': `${fontSize}px`, width: panelWidth, flexBasis: panelWidth } as CSSProperties}><div className="annotation-resize-handle" role="separator" aria-label={t('调整批注列表宽度')} title={t('拖动调整批注列表宽度')} onPointerDown={beginResize} onPointerMove={moveResize} onPointerUp={finishResize} onPointerCancel={finishResize} onLostPointerCapture={finishResize} /><div className="annotation-heading"><div><h2>{t('批注列表')}</h2><p>{message('annotation.count', { count: annotations.length })}{selectedIds.length ? ` · ${message('annotation.selected', { count: selectedIds.length })}` : ''}</p></div><div className="annotation-heading-actions"><button type="button" className="annotation-collapse" onClick={onToggle} title={t('收起批注列表')} aria-label={t('收起批注列表')}><span>›</span> {t('收起')}</button></div></div>
    <div className="annotation-toolbar"><span>{t('列表字号')}</span><div className="annotation-font-stepper"><button type="button" disabled={fontSize <= 10} onClick={() => setFontSize((value) => Math.max(10, value - 1))} aria-label={t('减小批注列表字号')} title={t('减小字号')}>A−</button><output>{fontSize}</output><button type="button" disabled={fontSize >= 15} onClick={() => setFontSize((value) => Math.min(15, value + 1))} aria-label={t('增大批注列表字号')} title={t('增大字号')}>A＋</button></div><button type="button" className="annotation-line-toggle" aria-pressed={singleLine} title={t(singleLine ? '切换为完整多行显示' : '切换为紧凑单行显示')} onClick={() => setSingleLine((value) => !value)}><span>{singleLine ? '☰' : '≡'}</span>{t(singleLine ? '多行' : '单行')}</button></div>
    ...
    <div className="annotation-list">{annotations.length ? annotations.map((annotation) => <AnnotationRow key={annotation.id} annotation={annotation} selected={selectedIds.includes(annotation.id) || annotation.id === selectedId} onSelect={(options) => onSelect(annotation, options)} onEdit={(content) => onEdit(annotation.id, content)} onColor={(color) => onColor(annotation.id, color)} onReply={(reply) => onReply(annotation.id, reply)} />) : <div className="empty-list">{t('还没有批注')}<br /><small>{t('在页面上框选文字开始批注')}</small></div>}</div>
    <div className="annotation-actions"><button onClick={() => onDelete(selectedIds.length ? selectedIds : selected ? [selected.id] : [])} disabled={!selectedIds.length && !selected} className="danger">{t('删除')}{selectedIds.length > 1 ? ` ${selectedIds.length}` : ''}{t('批注')}</button></div>
  </aside>
}
