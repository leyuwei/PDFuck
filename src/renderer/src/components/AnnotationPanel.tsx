...
function AnnotationRow({ annotation, selected, onSelect, onEdit, onColor, onReply }: { annotation: AnnotationRecord; selected: boolean; onSelect(options?: { additive?: boolean; range?: boolean }): void; onEdit(content: string): Promise<void>; onColor(color: string): Promise<void>; onReply(reply?: AnnotationReply): Promise<void> }) {
  ...
  return <div ref={rowRef} className={`annotation-row${selected ? ' selected' : ''}${replyClass}${settings ? ' settings-open' : ''}`} style={customStyle} onClick={(event) => onSelect({ additive: event.metaKey || event.ctrlKey, range: event.shiftKey })} onDoubleClick={() => setEditing(true)} onContextMenu={(event) => { event.preventDefault(); event.stopPropagation(); onSelect({ additive: event.metaKey || event.ctrlKey, range: event.shiftKey }); setSettings(true) }}>
    ...
    <span className="annotation-content">{editing ? <textarea autoFocus value={value} onChange={(event) => setValue(event.target.value)} onBlur={() => void commit()} onKeyDown={(event) => { if ((event.ctrlKey || event.metaKey) && event.key === 'Enter') void commit(); if (event.key === 'Escape') { setValue(original.current); setEditing(false) } }} onClick={(event) => event.stopPropagation()} /> : annotationContent(annotation, t)}</span>
